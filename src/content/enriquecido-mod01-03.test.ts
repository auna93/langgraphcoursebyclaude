...
/**
 * Tests de contrato — SE1 (SLICES.md §SE1): "Piloto: enriquecer mod01–03".
 * Cubre CA-30..CA-39 (PRD §12.5) EXCLUSIVAMENTE para mod01, mod02 y mod03,
 ...
 *
 * Antes de que SE1 enriquezca mod01–03 (campo `enriquecido: true` + `pasos` +
...
describe("SE1 — marcador `enriquecido` (ADR-15)", () => {
 ...
 *  LangGraph/langchain, más el cliente LLM local admitido (ADR-14). SE1 solo
 *  usa shim CORE (SLICES.md §SE1: "Solo shim core"), pero el allowlist general
...
describe("CA-36 — fidelidad de API del tutorial local (allowlist shim + ChatOllama, 0 cloud)", () => {
  ...
  it.each(MOD_IDS)("%s (SE1: solo shim core): sin ChatOllama ni superficie avanzada — mod01–03 no usan LLM", (id) => {
    const mod = requireModule(id);
    for (const bloque of mod.tutorialLocal?.codigo ?? []) {
      expect(bloque.codigo, `${id}/${bloque.archivo} no debería usar ChatOllama (mod01–03 sin LLM)`).not.toContain(
        CHAT_OLLAMA_SYMBOL,
      );
      for (const advanced of ADVANCED_SYMBOLS) {
        expect(bloque.codigo, `${id}/${bloque.archivo} usa símbolo avanzado "${advanced}" (SE1 es solo shim core)`).not.toMatch(
...
// ---------- CA-38: project spine continuo ----------

describe("CA-38 — project spine continuo (mod01 scaffolding; sin saltos)", () => {
  it("mod01 declara spine.scaffolding === true y crea el árbol inicial del proyecto", () => {
    const mod01 = requireModule("mod01");
    const spine = mod01.tutorialLocal?.spine;
    expect(spine, "mod01 debería definir tutorialLocal.spine").toBeDefined();
    expect(spine!.scaffolding, "mod01.tutorialLocal.spine.scaffolding debería ser true").toBe(true);
    expect(spine!.crea.length, "mod01 spine.crea debería incluir el scaffolding inicial").toBeGreaterThan(0);
  });

  it("mod02/mod03: NO tienen scaffolding === true (solo mod01)", () => {
    for (const id of ["mod02", "mod03"] as ModuleId[]) {
      const mod = requireModule(id);
      expect(mod.tutorialLocal?.spine.scaffolding, `${id} no debería declarar scaffolding`).not.toBe(true);
    }
  });
  ...
  it("continuidad sin saltos: todo archivo modificado por mod02/mod03 fue creado antes (o en el mismo módulo)", () => {
...
describe("CA-39 — el enriquecimiento no rompe CA-02/03/28 en mod01–03", () => {
